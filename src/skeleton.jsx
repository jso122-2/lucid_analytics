...
function init() {
  ...
  orbitControllerDiv.style.background = "#fff"; // white background for visibility
  orbitControllerDiv.style.border = "2px solid #000"; // optional border for contrast
  ...
  // Create a label above the orbit controller.
  ...
  orbitLabel.style.position = "absolute";
  orbitLabel.style.bottom = "170px"; // positions it above the orbit controller
  orbitLabel.style.left = "10px";
  orbitLabel.style.fontSize = "1rem";
  orbitLabel.style.color = "#000"; // dark text for contrast on the light background
  orbitLabel.style.fontWeight = "bold";
  orbitLabel.style.zIndex = "10002";
  ...
  scene.add(pointLight);
    ...
    // Create swirling particles for the text sequence.
    swirlParticles = createSwirlParticles();
    // Position the swirl particles at the center of the scene.
    swirlParticles.position.set(0, 0, 0);
    ...
    if (currentArticleIndex === 0 && currentModel === "skeleton" && skeletonHand?.visible) {
      ...
      gsap.to(articleContainer, {
        ...
        onComplete: () => {
          ...
          // Remove the transform button after sequence completion
          const transformBtn = document.getElementById("transform-btn");
          if (transformBtn) {
            transformBtn.remove();
          }
...
function createSwirlParticles() {
  ...
  // Create particles in a ring (with slight z-variation) around the center.
  for (let i = 0; i < count; i++) {
    const angle = Math.random() * Math.PI * 2;
    const radius = 1.5 + Math.random() * 0.5; // ring radius between 1.5 and 2.0
    positions[i * 3] = radius * Math.cos(angle);
    positions[i * 3 + 1] = radius * Math.sin(angle);
    positions[i * 3 + 2] = (Math.random() - 0.5) * 0.5; // small z variation
  }
  geometry.setAttribute("position", new THREE.BufferAttribute(positions, 3));
...
function animate() {
  ...
  if (swirlParticles) {
    // Slowly rotate the particles around the Z-axis.
    swirlParticles.rotation.z += 0.005;
  }
